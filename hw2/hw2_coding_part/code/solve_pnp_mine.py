...
def PnP(Pc, Pw, K=np.eye(3)):
    ...
    # The Z-component of the translation vector must be positive.
    # If it's negative, the tag is behind the camera, so we flip the signs.
    if t_wc[2] < 0:
        r1 = -r1
        r2 = -r2
        t_wc = -t_wc
    ...
    # Use SVD to find the closest valid rotation matrix.
